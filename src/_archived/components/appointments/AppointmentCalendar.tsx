...
export function AppointmentCalendar({ onDateSelect, onAvailabilityLoad, clinicId }: Props) {
  ...
  // Fetch availability when date or clinic changes
  useEffect(() => {
    if (date) {
    ...
    <Card>
      ...
      <CardContent>
          ...
          <div className="mt-4 p-3 bg-muted rounded-md space-y-2">
            ...
            {!availability.closed && availability.hours && (
              <p className="text-xs text-muted-foreground">
                Horario: {availability.hours.display || `${availability.hours.open} - ${availability.hours.close}`}
              </p>
            )}
        ...
        <p className="text-sm text-muted-foreground mt-4">
          * Domingos cerrados. El horario puede variar segun la clinica seleccionada.
        </p>
      </CardContent>
    </Card>
  );
